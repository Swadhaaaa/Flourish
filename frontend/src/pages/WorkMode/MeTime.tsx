...
// Hardcoded events removed in favor of API
...
export default function MeTime() {
    ...
    useEffect(() => {
        const fetchEvents = async () => {
            try {
                const res = await fetch(`${API_URL}/api/events/nearby?category=${filter === 'All' ? '' : filter}`);
                if (res.ok) {
    ...
    const getColor = (cat: string) => {
        switch (cat) {
            ...
            case 'Creative': return "bg-orange-100 text-orange-600";
            case 'Music': return "bg-indigo-100 text-indigo-600";
                                ...
                                >
                                    ...
                                    <div className="h-48 overflow-hidden relative">
                                        ...
                                        <div className="absolute top-4 right-4 z-20 flex gap-2">
                                            {event.source && (
                                                <div className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border border-white/20 backdrop-blur-md ${event.source === 'simulated' ? 'bg-slate-500/50 text-white' : 'bg-pink-500/80 text-white'}`}>
                                                    {event.source}
                                                </div>
                                            )}
